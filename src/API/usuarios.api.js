...
const PROTOCOL_SERVER = "https://";
const HOST_SERVER = "taller-deiber-amazon.tk";
const PORT_SERVER = 3009;
// import {useBook} from '../context/BookContext'
// const {token} = useBook()
export const createUserRequiest = async (user) => {
  const response =   await axios.post(
    `${PROTOCOL_SERVER}${HOST_SERVER}/api/v1/libary/usuarios`,
    user
  );
  return response
};
...
export const redireccionVerificacionEmailRequiest = async () => {
  const response = await axios.get(
    `${PROTOCOL_SERVER}${HOST_SERVER}/api/v1/libary/usuarios/confirmar_cuenta?`
  );
  return response;
};
...
export const inicioDeSessionRequest = async (user) => {
  const { data } = await axios.post(
    `${PROTOCOL_SERVER}${HOST_SERVER}/api/v1/libary/usuarios/iniciar_sesion`,
    user
  );
  console.log(data);
